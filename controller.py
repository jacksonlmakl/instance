...
class EC2Manager:
    ...
    def launch(self, instance_id=None):
        ...
        try:
            result = self._execute_commands(
                instance_id=instance_id,
                commands=["""cd manager && (bash ~/manager/launch > ~/launch.log 2>&1 &) < /dev/null ""","""cd manager && (bash ~/manager/bin/stop > ~/stop.log 2>&1 &) < /dev/null ""","""cd manager && (bash ~/manager/bin/start > ~/start.log 2>&1 &) < /dev/null """]
            )
